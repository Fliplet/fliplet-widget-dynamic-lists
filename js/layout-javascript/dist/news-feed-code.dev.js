...
function DynamicList(id, data) {
  ...
  this.src = this.data.advancedSettings && this.data.advancedSettings.detailHTML ? this.data.advancedSettings.detailHTML : Fliplet.Widget.Templates[_this.layoutMapping[this.data.layout]['detail']](); // Register handlebars helpers
...
DynamicList.prototype.attachObservers = function () {
  ...
  _this.$container.on('click', '[data-lfd-back]', function () {
    var result;
    ...
    _this.Utils.DOM.resetSortIcons({
      $sortList: $sortList
    });
    ...
    _this.$container.find('.hidden-filter-controls').animate({
      height: 0
    }, 200); // Clear filters
    ...
    if (Modernizr.ios) {
      setTimeout(function () {
        _that.parents('.new-news-feed-comment-panel').addClass('typing'); // Adds binding
    ...
    if (Modernizr.ios) {
      setTimeout(function () {
        ...
        window.scrollTo(0, 0); // Removes binding
...
DynamicList.prototype.initializeOverlaySocials = function (id) {
  ...
  return Promise.all([bookmarkPromise, likePromise, _this.getEntryComments({
    id: record.id,
    record: record
  })]);
};
...
DynamicList.prototype.getCommentUsers = function () {
  ...
  var _this = this; // Get users info for comments


  return _this.connectToUsersDataSource().then(function (users) {
    ...
    _this.allUsers = users; // Update my user data
...
DynamicList.prototype.initialize = function () {
  ...
  return (shouldInitFromQuery ? Promise.resolve() : _this.parsePVQueryVars()).then(function () {
    ...
    return Fliplet.Hooks.run('flListDataAfterGetData', {
      ...
      return _this.Utils.Records.setFilterValues(_this.data);
    }).then(function () {
...
DynamicList.prototype.renderBaseHTML = function () {
  ...
  data.previousScreen = _this.pvPreviousScreen; // go back to previous screen on click - TRUE/FALSE
...
DynamicList.prototype.renderLoopHTML = function () {
  ...
  var template = _this.data.advancedSettings && _this.data.advancedSettings.loopHTML ? Handlebars.compile(_this.data.advancedSettings.loopHTML) : Handlebars.compile(Fliplet.Widget.Templates[_this.layoutMapping[_this.data.layout]['loop']]());
  var limitedList;

  if (_this.data.enabledLimitEntries && _this.data.limitEntries >= 0 && !_this.isSearching && !_this.isFiltering && !_this.showBookmarks) {
    limitedList = _this.modifiedListItems.slice(0, _this.data.limitEntries); // Hides the entry limit warning if the number of entries to show is less than the limit value
  ...
  return new Promise(function (resolve) {
    function render() {
      ...
      var nextBatch = data.slice(renderLoopIndex * _this.INCREMENTAL_RENDERING_BATCH_SIZE, renderLoopIndex * _this.INCREMENTAL_RENDERING_BATCH_SIZE + _this.INCREMENTAL_RENDERING_BATCH_SIZE);
      ...
      if (nextBatch.length) {
        ...
        _this.$container.find('.new-news-feed-list-container').removeClass('loading').addClass('ready'); // Changing close icon in the fa-times-thin class for windows 7 IE11
...
DynamicList.prototype.searchData = function (options) {
  ...
  var limit = limitEntriesEnabled && _this.data.limitEntries > -1 && !_this.isSearching && !_this.showBookmarks && !_this.isFiltering ? _this.data.limitEntries : -1;
  ...
  return _this.Utils.Records.runSearch({
    ...
    return Fliplet.Hooks.run('flListDataBeforeRenderList', {
      ...
      _this.$container.find('.new-news-feed-list-container').removeClass('searching'); // Adds search query to HTML
      ...
      if (!_this.data.forceRenderList && searchedData.length && searchedData.length === _.intersectionBy(searchedData, _this.searchedListItems, 'id').length) {
...
DynamicList.prototype.setupLikeButton = function (options) {
  ...
  return _this.getLikeIdentifier(record).then(function (identifier) {
    return new Promise(function (resolve) {
      var btn = LikeButton({
        target: target,
...
DynamicList.prototype.addDetailViewData = function (entry) {
  ...
  entry.entryDetails = []; // Define detail view data based on user's settings
...
DynamicList.prototype.showDetails = function (id, listData) {
  ...
  var entryData = _.find(listData || _this.modifiedListItems, {
    id: id
  ...
  return beforeShowDetails.then(function (data) {
    ...
    _this.$container.find('.new-news-feed-list-container').addClass('overlay-open'); // Calculate top position when image finishes loading
    ...
    if ($(window).width() < 640) {
      _this.$container.find('.news-feed-list-detail-image-wrapper img').one('load', function () {
        ...
        _this.$overlay.find('.news-feed-item-inner-content').css({
          top: expandedPosition + 'px'
        });
      }).each(function () {
        if (this.complete) {
          $(this).trigger('load');
...
DynamicList.prototype.closeDetails = function () {
  ...
  setTimeout(function () {
    // Clears overlay
    _this.$overlay.find('.news-feed-detail-overlay-content-holder').html(''); // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
...
DynamicList.prototype.getEntryComments = function (options) {
  ...
  return _this.getCommentIdentifier(record).then(function (identifier) {
    var getComments = Promise.resolve();

    if (typeof count === 'undefined' || options.force) {
      getComments = Fliplet.Content({
        dataSourceId: _this.data.commentsDataSourceId
      }).then(function (instance) {
        return instance.query({
          allowGrouping: true,
          where: {
...
DynamicList.prototype.connectToUsersDataSource = function () {
  ...
  return Fliplet.DataSources.connect(_this.data.userDataSourceId, options).then(function (connection) {
...
DynamicList.prototype.updateCommentCounter = function (options) {
  ...
  var html = counterCompiled(data); // Updates both main list and overlay comment counters
...
DynamicList.prototype.showComments = function (id, commentId) {
  ...
  return _this.getCommentUsers().then(function () {
    ...
    var entry = _.find(_this.listItems, {
      id: id
    });

    var entryComments = _.get(entry, 'comments'); // Display comments
...
DynamicList.prototype.sendComment = function (id, value) {
  var record = _.find(this.listItems, {
    id: id
  });
  ...
  return Fliplet.Hooks.run('flListDataBeforeNewComment', options).then(function () {
    ...
    userName = _.compact(_.map(_this.data.userNameFields, function (name) {
      return _this.myUserData.isSaml2 ? _.get(userFromDataSource, 'data.' + name) : _this.myUserData[name];
    })).join(' ').trim();
    var comment = {
      fromName: userName,
    ...
    var timestamp = new Date().toISOString(); // Get mentioned user(s)
    ...
    return _this.getCommentIdentifier(record).then(function (identifier) {
      return Fliplet.Profile.Content({
        dataSourceId: _this.data.commentsDataSourceId
      }).then(function (instance) {
      ...
      return Fliplet.Hooks.run('flListDataAfterNewComment', options).then(function () {
...
DynamicList.prototype.deleteComment = function (id) {
  ...
  return Fliplet.Hooks.run('flListDataBeforeDeleteComment', options).then(function () {
    return Fliplet.DataSources.connect(_this.data.commentsDataSourceId).then(function (connection) {
      return connection.removeById(id, {
        ack: true
      });
    }).then(function onRemove() {
      _.remove(entry.comments, {
        id: id
      });
...
DynamicList.prototype.saveComment = function (entryId, commentId, newComment) {
  ...
  var commentData = _.find(entryComments, {
    id: commentId
  });
  ...
  return Fliplet.Hooks.run('flListDataBeforeUpdateComment', options).then(function () {
    ...
    return Fliplet.Content({
      ...
      return Fliplet.Hooks.run('flListDataAfterUpdateComment', options).then(function () {
