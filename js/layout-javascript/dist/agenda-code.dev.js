...
function DynamicList(id, data) {
  ...
  this.$container = $('[data-dynamic-lists-id="' + id + '"]'); // Other variables
  // Global variables
  ...
  this.src = this.data.advancedSettings && this.data.advancedSettings.detailHTML ? this.data.advancedSettings.detailHTML : Fliplet.Widget.Templates[_this.layoutMapping[this.data.layout]['detail']]();
  this.detailHTML = Handlebars.compile(this.src); // Register handlebars helpers

  this.Utils.registerHandlebarsHelpers(); // Get the current session data
...
DynamicList.prototype.goToAgendaFeature = function (options) {
  ...
  var screen = _.find(Fliplet.Env.get('appPages'), {
    title: screenName
  });
  ...
  return Fliplet.App.Storage.set(type + 'SessionTitle-' + screen.id, data).then(function () {
...
DynamicList.prototype.attachObservers = function () {
  var _this = this; // Attach your event listeners here
  ...
  _this.$container.on('click', '.apply-filters', function () {
    ...
    $('body').removeClass('lock has-filter-overlay'); // Clear all selected filters

    _this.toggleFilterElement(_this.$container.find('.mixitup-control-active:not(.toggle-bookmarks)'), false); // No filters selected
    ...
    _this.calculateFiltersHeight(true); // Clear filters
    ...
    Fliplet.Analytics.trackEvent({
      category: 'list_dynamic_' + _this.data.layout,
      action: $toggle.hasClass('mixitup-control-active') ? 'bookmarks_show' : 'bookmarks_hide'
    });
    ...
    $(this).removeClass('hover'); // Delay to compensate for the fast click event
    ...
    var options = {
      ...
      labels: [{
        ...
        action: function action() {
          _that.text('Deleting...').addClass('disabled'); // Run Hook
    ...
    var record = _.find(_this.listItems, {
      id: id
    });
...
DynamicList.prototype.initialize = function () {
  ...
  var shouldInitFromQuery = _this.parseQueryVars(); // query will always have higher priority than storage
  ...
  _this.attachObservers(); // Check if there is a query or PV for search/filter queries


  return (shouldInitFromQuery ? Promise.resolve() : _this.parsePVQueryVars()).then(function () {
...
DynamicList.prototype.parsePVQueryVars = function () {
  ...
  var pvValue;
  return Fliplet.App.Storage.get('flDynamicListQuery:' + _this.data.layout).then(function (value) {
    pvValue = value;
...
DynamicList.prototype.parseFilterQueries = function () {
  ...
  var filterSelectors = _this.Utils.Query.getFilterSelectors({
    query: _this.pvFilterQuery
  });
...
DynamicList.prototype.connectToDataSource = function () {
  ...
  function getData(options) {
    ...
    return Fliplet.DataSources.connect(_this.data.dataSourceId, options).then(function (connection) {
      ...
      } else if (_typeof(_this.data.dataQuery) === 'object') {
        query = _this.data.dataQuery;
      }
...
DynamicList.prototype.renderBaseHTML = function () {
  ...
  var data = _this.getAddPermission(_this.data); // go to previous screen on close detail view - TRUE/FALSE
  ...
  var template = _this.data.advancedSettings && _this.data.advancedSettings.baseHTML ? Handlebars.compile(_this.data.advancedSettings.baseHTML) : Handlebars.compile(baseHTML());
...
DynamicList.prototype.groupLoopDataByDate = function (loopData, dateField) {
  var _this = this; // Group data by date field
  ...
  var recordDates = _.orderBy(_.keys(recordGroups)); // Prepare a merge if the date values are parsed as the same date
...
DynamicList.prototype.addSummaryData = function (records) {
  ...
  var loopData = _.map(modifiedData, function (entry) {
    var newObject = {
      ...
      pollButton: _this.data.pollEnabled && entry.data[_this.data.pollColumn] && entry.data[_this.data.pollColumn] !== '',
      surveyButton: _this.data.surveyEnabled && entry.data[_this.data.surveyColumn] && entry.data[_this.data.surveyColumn] !== '',
      questionsButton: _this.data.questionsEnabled && entry.data[_this.data.questionsColumn] && entry.data[_this.data.questionsColumn] !== '',
      agendaButtonsEnabled: _this.data.pollEnabled && entry.data[_this.data.pollColumn] && entry.data[_this.data.pollColumn] !== '' || _this.data.surveyEnabled && entry.data[_this.data.surveyColumn] && entry.data[_this.data.surveyColumn] !== '' || _this.data.questionsEnabled && entry.data[_this.data.questionsColumn] && entry.data[_this.data.questionsColumn] !== '',
    ...
    var dateField = _.find(_this.data.detailViewOptions, {
      location: _this.dateFieldLocation
    });
...
DynamicList.prototype.renderLoopHTML = function () {
  ...
  if (this.isInLoopView()) {
    template = _this.data.advancedSettings && _this.data.advancedSettings.loopHTML ? Handlebars.compile(_this.data.advancedSettings.loopHTML) : Handlebars.compile(Fliplet.Widget.Templates[_this.layoutMapping[_this.data.layout]['loop']]());
  } else {
    template = _this.data.advancedSettings && _this.data.advancedSettings.searchResultsHTML ? Handlebars.compile(_this.data.advancedSettings.searchResultsHTML) : Handlebars.compile(Fliplet.Widget.Templates[_this.layoutMapping[_this.data.layout]['search-results']]());
  }
  ...
  var $renderFull = $([]);
  var $agendaListHolder = _this.isInLoopView() ? $('#agenda-cards-wrapper-' + _this.data.id + ' .agenda-list-holder') : _this.$container.find('.search-results-wrapper .search-results-holder');
  ...
  return new Promise(function (resolve) {
    function render() {
      ...
      var nextBatch = _this.getAgendasByDay().slice(_this.INCREMENTAL_RENDERING_BATCH_SIZE * renderLoopIndex, _this.INCREMENTAL_RENDERING_BATCH_SIZE * (renderLoopIndex + 1)); // Break render cycle if there is no more data
      ...
      renderLoopIndex++; // if the browser is ready, render
...
DynamicList.prototype.renderDatesHTML = function (records, index) {
  ...
  var foundDateField = _.find(_this.data.detailViewOptions, {
    location: _this.dateFieldLocation
  });

  var dateField = _.get(foundDateField, 'column');

  if (!dateField || _this.dataSourceColumns.indexOf(dateField) === -1) {
    throw new Error('Date field is misconfigured. Please check your component settings.');
  }

  var clonedRecords = _.clone(records); // Keep only records with valid dates when rendering dates selectors
  ...
  if (clonedRecords.length) {
    ...
    firstDate = _this.Utils.Date.moment(clonedRecords[0].data[dateField]); // Set last date in agenda

    lastDate = _this.Utils.Date.moment(clonedRecords[clonedRecords.length - 1].data[dateField]); // Adds (numberOfPlaceholderDays) days before the first date
    ...
    }), 'data.' + dateField); // Get the event dates
    // Save in an array
    ...
    this.agendaDates = _.orderBy(this.agendaDates); // Adds (numberOfPlaceholderDays) days after the last date
    // Save them in an array
  ...
  var template = this.data.advancedSettings && this.data.advancedSettings.otherLoopHTML ? Handlebars.compile(this.data.advancedSettings.otherLoopHTML) : Handlebars.compile(Fliplet.Widget.Templates[this.layoutMapping[this.data.layout]['other-loop']]());
  this.$container.find('.agenda-date-selector ul').html(template(calendarDates)); // Selects the first date
...
DynamicList.prototype.getPermissions = function (entries) {
  var _this = this; // Adds flag for Edit and Delete buttons
...
DynamicList.prototype.addFilters = function (records) {
  ...
  return Fliplet.Hooks.run('flListDataBeforeRenderFilters', {
    ...
    var template = _this.data.advancedSettings && _this.data.advancedSettings.filterHTML ? Handlebars.compile(_this.data.advancedSettings.filterHTML) : Handlebars.compile(filtersTemplate());
...
DynamicList.prototype.getActiveFilters = function () {
  return _(this.$container.find('.hidden-filter-controls-filter.mixitup-control-active')).map(function (el) {
    return _.pickBy({
      "class": el.dataset.toggle,
      field: el.dataset.field,
      value: el.dataset.value
    });
  }).groupBy('field').mapValues(function (filters) {
    return _.map(filters, function (filter) {
      return _.has(filter, 'field') && _.has(filter, 'value') ? filter.value : filter["class"];
    });
  }).value();
};
...
DynamicList.prototype.calculateFiltersHeight = function (hideFilters) {
  ...
  var totalHeight = hideFilters ? 0 : this.$container.find('.hidden-filter-controls-content').height();
  return new Promise(function (resolve) {
...
DynamicList.prototype.calculateSearchHeight = function (clearSearch) {
  ...
  var totalHeight = clearSearch ? 0 : this.$container.find('.hidden-search-controls-content').height();
  return new Promise(function (resolve) {
...
DynamicList.prototype.initializeSocials = function (records) {
  ...
  return _this.getAllBookmarks().then(function () {
    return Promise.all(_.map(_.flatten(records), function (record) {
      ...
      var masterRecord = _.find(_this.listItems, {
        id: record.id
      });

      var $listView = _this.isInLoopView() ? _this.$container.find('.agenda-cards-wrapper') : _this.$container.find('.search-results-wrapper');
...
DynamicList.prototype.animateDateForward = function (nextDateElement, nextDateElementWidth) {
  ...
  return new Promise(function (resolve) {
    ...
    }, _this.ANIMATION_SPEED, 'swing', // animation easing
    function () {
...
DynamicList.prototype.moveForwardDate = function (index, difference) {
  ...
  Promise.all([_this.animateDateForward(nextDateElement, nextDateElementWidth), _this.animateAgendaForward(nextAgendaElement, nextAgendaElementWidth - _this.scrollValue)]).then(function () {
...
DynamicList.prototype.moveBackDate = function (index, difference) {
  ...
  Promise.all([_this.animateDateBack(prevDateElement, prevDateElementWidth), _this.animateAgendaBack(prevAgendaElement, prevAgendaElementWidth + _this.scrollValue)]).then(function () {
    _this.isPanning = false;
...
DynamicList.prototype.toggleBookmarkStatus = function (record) {
  ...
  if (record.bookmarked) {
    this.$container.find('.agenda-cards-wrapper, .search-results-wrapper').find('.agenda-item-bookmark-holder-' + record.id + ' .bookmark-wrapper.btn-bookmark').removeClass('btn-bookmark').addClass('btn-bookmarked').find('.fa-bookmark-o').removeClass('fa-bookmark-o').addClass('fa-bookmark');
  } else {
    this.$container.find('.agenda-cards-wrapper, .search-results-wrapper').find('.agenda-item-bookmark-holder-' + record.id + ' .bookmark-wrapper.btn-bookmarked').removeClass('btn-bookmarked').addClass('btn-bookmark').find('.fa-bookmark').removeClass('fa-bookmark').addClass('fa-bookmark-o');
  }
};
...
DynamicList.prototype.setupBookmarkButton = function (options) {
  ...
  var record = options.record || _.find(_this.listItems, {
    id: id
  });
  ...
  return _this.getBookmarkIdentifier(record).then(function (identifier) {
    return new Promise(function (resolve) {
      var btn = LikeButton({
        target: target,
        dataSourceId: _this.data.bookmarkDataSourceId,
...
DynamicList.prototype.removeFilteredEntries = function (data) {
  if (this.isInLoopView()) {
    ...
    var recordIdsToHide = _.difference(_.map(this.filteredListItemsByDay, 'id'), recordIdsToShow); // Hide and show content based on existing render
...
DynamicList.prototype.searchData = function (options) {
  ...
  return _this.Utils.Records.runSearch({
    ...
    return Fliplet.Hooks.run('flListDataBeforeRenderList', {
      ...
      _this.$container.find('.new-agenda-list-container').removeClass('searching'); // Adds search query to HTML


      _this.$container.find('.current-query').html(_this.searchValue); // Search value is provided
      ...
      _this.setAgendasByDay(_this.groupLoopDataByDate(_this.modifiedListItems, _this.dateFieldLocation)); // Render the full list
...
DynamicList.prototype.sliderGoTo = function (number) {
  ...
  var _this = this; // Stop it from doing weird things like moving to slides that don’t exist
...
DynamicList.prototype.addDetailViewData = function (entry) {
  ...
  entry.entryDetails = []; // Uses detail view settings not set by users
...
DynamicList.prototype.showDetails = function (id, listData) {
  ...
  var entryData = _.find(listData, {
    id: id
  }) || _(_this.getAgendasByDay()).chain().thru(function (coll) {
  ...
  return beforeShowDetails.then(function (data) {
    ...
    var wrapperTemplate = Handlebars.compile(wrapper); // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
    ...
    _this.initializeOverlaySocials(id); // Trigger animations
...
DynamicList.prototype.closeDetails = function () {
  ...
  $('body').removeClass('lock'); // Sets up the like and bookmark buttons
  ...
  setTimeout(function () {
    $overlay.removeClass('ready'); // Clears overlay

    $overlay.find('.agenda-detail-overlay-content-holder').html(''); // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
