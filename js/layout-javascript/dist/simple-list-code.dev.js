...
function DynamicList(id, data) {
  ...
  this.data.bookmarksEnabled = _.get(this, 'data.social.bookmark'); // Register handlebars helpers
...
DynamicList.prototype.attachObservers = function () {
  ...
  _this.$container.on('click', '[data-lfd-back]', function () {
    ...
    if (Modernizr.ios) {
      setTimeout(function () {
        _that.parents('.simple-list-comment-panel').addClass('typing'); // Adds binding
  ...
  return new Promise(function (resolve) {
    function render() {
      ...
      if (nextBatch.length) {
        ...
        _this.$container.find('.simple-list-container').removeClass('loading').addClass('ready'); // Changing close icon in the fa-times-thin class for windows 7 IE11
...
DynamicList.prototype.searchData = function (options) {
  ...
  return _this.Utils.Records.runSearch({
    ...
    return Fliplet.Hooks.run('flListDataBeforeRenderList', {
      ...
      _this.$container.find('.simple-list-container').removeClass('searching'); // Adds search query to HTML
...
DynamicList.prototype.addDetailViewData = function (entry) {
  ...
  entry.data = []; // Define detail view data based on user's settings
...
DynamicList.prototype.showDetails = function (id, listData) {
  ...
  var src = _this.data.advancedSettings && _this.data.advancedSettings.detailHTML ? _this.data.advancedSettings.detailHTML : Fliplet.Widget.Templates[_this.layoutMapping[_this.data.layout]['detail']]();
...
DynamicList.prototype.closeDetails = function () {
  ...
  setTimeout(function () {
    // Clears overlay
    $overlay.find('.simple-list-detail-overlay-content-holder').html(''); // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
