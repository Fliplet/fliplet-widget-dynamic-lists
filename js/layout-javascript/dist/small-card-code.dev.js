...
function DynamicList(id, data) {
  ...
  this.data.bookmarksEnabled = _this.data.social.bookmark; // Register handlebars helpers
...
DynamicList.prototype.attachObservers = function () {
  ...
  _this.$container.on('click', '[data-lfd-back]', function () {
    ...
    _this.toggleFilterElement(_this.$container.find('.mixitup-control-active:not(.toggle-bookmarks)'), false); // Select filters based on existing settings
...
DynamicList.prototype.initialize = function () {
  ...
  return (shouldInitFromQuery ? Promise.resolve() : _this.parsePVQueryVars()).then(function () {
    ...
    records = _this.getPermissions(records); // Get user profile
  ...
  return new Promise(function (resolve) {
    function render() {
      ...
      if (nextBatch.length) {
        ...
        _this.$container.find('.new-small-card-list-container').removeClass('loading').addClass('ready'); // Changing close icon in the fa-times-thin class for windows 7 IE11
...
DynamicList.prototype.searchData = function (options) {
  ...
  return _this.Utils.Records.runSearch({
    ...
    return Fliplet.Hooks.run('flListDataBeforeRenderList', {
      ...
      _this.$container.find('.new-small-card-list-container').removeClass('searching'); // Adds search query to HTML
      ...
      _this.$container.find('.hidden-search-controls').addClass('active')[searchedData.length || truncated ? 'removeClass' : 'addClass']('no-results');
      ...
      return _this.renderLoopHTML().then(function (records) {
        _this.searchedListItems = searchedData; // Render user profile
...
DynamicList.prototype.closeDetails = function () {
  ...
  setTimeout(function () {
    ...
    $overlay.find('.small-card-detail-overlay-content-holder').html(''); // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
...
DynamicList.prototype.expandElement = function (elementToExpand, id, listData) {
  ...
  var _this = this; // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
  ...
  if (!elementToExpand.hasClass('open')) {
    ...
    var directoryDetailImage = elementToExpand.find('.small-card-list-detail-image'); // Get the size of the html offset
    ...
    var toTop = parseInt(htmlMarginTop, 10); // convert the expand-item to fixed position with a high z-index without moving it
...
DynamicList.prototype.collapseElement = function (elementToCollapse) {
  ...
  directoryDetailImage.animate({
    ...
    elementToCollapse.parents('.small-card-list-item').removeClass('opening'); // This bit of code will only be useful if this component is added inside a Fliplet's Accordion component
