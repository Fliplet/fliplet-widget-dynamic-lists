...
var DynamicLists = function () {
  ...
  var tokenField = Fliplet.Widget.Templates['templates.interface.field-token']; // ADD NEW MAPPING FOR ALL NEW STYLES THAT ARE ADDED
  ...
  var addRadioValues = [];
  var editRadioValues = [];
  var deleteRadioValues = []; // Constructor
  ...
  DynamicLists.prototype = {
    ...
    toggleCustomImageFields: function toggleCustomImageFields($row, field, type) {
    ...
    attachListeners: function attachListeners() {
      ...
      $(document).on('click', '.layout-holder', function () {
        listLayout = $(this).data('layout');
        isLayoutSelected = true;
        $('.state.present').addClass('is-loading'); // Create data source

        _this.loadDataFromLayout().then(function () {
      ...
      }).on('click', '[data-create-datasource]', _this.createDataSourceData).on('click', '#manage-data, [data-edit-datasource]', _this.manageAppData).on('click', '[data-reset-default]', function () {
        ...
        if ($(this).is(':checked') && !_resetToDefaults) {
        ...
        switch (dataType) {
          case 'all-folders':
            $(this).parents('.rTableRow').find('.picker-provider-button, .folders-only').removeClass('hidden').end().find('.url-only').addClass('hidden');
            break;

          case 'url':
            $(this).parents('.rTableRow').find('.picker-provider-button, .folders-only').addClass('hidden').end().find('.url-only').removeClass('hidden');
            break;

          default:
            $(this).parents('.rTableRow').find('.picker-provider-button, .url-only').addClass('hidden').end().find('.folders-only').removeClass('hidden');
        }
    ...
    manageAppData: function manageAppData() {
    ...
    managePermissionList: function managePermissionList(options) {
      ...
      if (dataSourceProvider) {
        dataSourceProvider.emit('update-security-rules', {
          accessRules: accessRules
        });
      }
    },
    toggleUserEmail: function toggleUserEmail(showUserEmailList) {
      $('.select-user-email-list-holder').toggleClass('hidden', !showUserEmailList);
    },
    toggleAdminIndetification: function toggleAdminIndetification(showUserAdminHolder) {
      $('.select-user-admin-holder').toggleClass('hidden', !showUserAdminHolder);
    },
    updatePermission: function updatePermission(permissionType) {
      var controlValues = [];
      ...
      var isAdminsCanDeleteEntry = (canDeleteEntry === 'admins' || canDeleteEntry === 'users-admins') && isDeleteEntryActive;
      var isUsersCanEditEntry = (canEditEntry === 'user' || canEditEntry === 'users-admins') && isEditEntryActive;
      var isUsersCanDeleteEntry = (canDeleteEntry === 'user' || canDeleteEntry === 'users-admins') && isDeleteEntryActive;
      var showUserEmailList = isUsersCanEditEntry || isUsersCanDeleteEntry;
      var showUserAdminHolder = isAdminsCanAddEntry || isAdminsCanEditEntry || isAdminsCanDeleteEntry;
      var showUsersDataSource = isAdminsCanAddEntry || isAdminsCanEditEntry || isAdminsCanDeleteEntry || isUsersCanEditEntry || isUsersCanDeleteEntry || this.config.social && this.config.social.comments;
    ...
    initUserDatasourceProvider: function initUserDatasourceProvider(dataSourceId, showUsersDataSource) {
            ...
            switch (event) {
              case 'dataSourceSelect':
                ...
                if (_this.config.userDataSourceId === 'none' || _this.config.userDataSourceId === '') {
                  $('.select-user-firstname-holder, .select-user-lastname-holder, .select-user-email-holder, .select-user-photo-holder, .select-photo-folder-type, .select-user-admin-holder').addClass('hidden');
                  return;
                }
    ...
    toggleRuleTypes: function toggleRuleTypes(options) {
      var defaultRule = {
        allow: 'all',
        type: []
      };
    ...
    renderFilterColumns: function renderFilterColumns() {
    ...
    renderSortColumns: function renderSortColumns() {
    ...
    validateImageFieldOption: function validateImageFieldOption(value) {
      if (!value) {
        return 'url';
      }
    ...
    setupLayoutSelector: function setupLayoutSelector() {
    ...
    init: function init() {
      ...
      return _this.getDataSources().then(function () {
    ...
    setSourceValue: function setSourceValue(data) {
    ...
    loadData: function loadData() {
      ...
      if (!_this.config.dataSourceId) {
        ...
        loadingPromise = _this.getDataSourceById(_this.config.dataSourceId).then(function (datasource) {
      ...
      return loadingPromise.then(function () {
        ...
        _this.checkSortPanelLength(); // Load filter options
        ...
        $('#items-number').val(_this.config.limitEntries); // Load Search/Filter fields
        ...
        $('#enable-filter-overlay').prop('checked', _this.config.filtersInOverlay).trigger('change'); // Load agenda feature
        ...
        $('#select_questions_data').val(_this.config.questionsColumn || 'none'); // Load social feature
        ...
        listLayout = _this.config.layout; // eslint-disable-next-line no-unused-vars

        isLayoutSelected = true;
        $('.layout-holder[data-layout="' + _this.config.layout + '"]').addClass('active'); // init Data Source Provider

        initDataSourceProvider(_this.config.dataSourceId); // Load Add. Edit, Delete
        ...
        $('[name="delete-permissions"][value="' + deletePermission + '"]').prop('checked', true).trigger('change'); // Load code editor tabs
        ...
        dataSourceColumns = dataSourceColumns || _this.config.dataSourceColumns || _this.config.defaultColumns; // Sets up the data view settings
        ...
        var defaultLockedFields = _.filter(defaultDetailFields, {
          paranoid: true
        });
        ...
        if (defaultLockedFields.length) {
          ...
          if (foundLockedFields.length < defaultLockedFields.length) {
            ...
            foundLockedFields = _.map(defaultLockedFields, function (field) {
              return _.merge(field, _.find(foundLockedFields, {
                location: field.location
              }));
            }); // Prepend locked fields in the beginning

            _this.config.detailViewOptions = _.concat(foundLockedFields, _.filter(_this.config.detailViewOptions, function (option, index) {
        ...
        if (_this.config.social.comments || _this.config.userDataSourceId) {
          ...
          $('.select-photo-folder .selected-user-folder span').text(_this.config.userFolder && _this.config.userFolder.folder ? _this.config.userFolder.folder.selectFiles[0].name : '');
          $('.select-photo-folder .selected-user-folder')[_this.config.userFolder && _this.config.userFolder.folder ? 'removeClass' : 'addClass']('hidden');
        ...
        $('#enable-questions').prop('checked', _this.config.questionsEnabled); // Load social feature

        $('#enable-likes').prop('checked', _this.config.social.likes);
        $('#enable-bookmarks').prop('checked', _this.config.social.bookmark);
        $('#enable-comments').prop('checked', _this.config.social.comments); // Select layout

        listLayout = _this.config.layout;
        $('.layout-holder[data-layout="' + _this.config.layout + '"]').addClass('active'); // Load code editor tabs
        ...
        $('.create-holder').addClass('hidden');
        $('.edit-holder').removeClass('hidden');
        $('.form-group').removeClass('disabled'); // Continue
    ...
    updateSummaryRowContainer: function updateSummaryRowContainer() {
      ...
      _.forEach(_this.config['summary-fields'], function (item) {
        ...
        if (item.imageField === 'all-folders' && item.folder) {
          $summaryRowContainer.find('[data-id="' + item.id + '"]').find('.file-picker-btn').text('Replace folder').end().find('.selected-folder span').text(item.folder.selectFiles[0].name).end().find('.selected-folder').removeClass('hidden');
        }
      });
    },
    updateDetailsRowContainer: function updateDetailsRowContainer() {
      ...
      _.forEach(_this.config.detailViewOptions, function (item) {
        ...
        if (item.imageField === 'all-folders' && item.folder) {
          $detailsRowContainer.find('[data-id="' + item.id + '"]').find('.file-picker-btn').text('Replace folder').end().find('.selected-folder span').text(item.folder.selectFiles[0].name).end().find('.selected-folder').removeClass('hidden');
        }
      });
    },
    loadTokenFields: function loadTokenFields() {
    ...
    setUpTokenFields: function setUpTokenFields() {
    ...
    removeFocusFromTokenInput: function removeFocusFromTokenInput() {
      $('input.token-input.ui-autocomplete-input').blur();
    },
    handleTokensSelection: function handleTokensSelection() {
      $('input.tokenfield').on('tokenfield:createdtoken tokenfield:removedtoken', function () {
        var field = $(this);
        var currentTokens = field.tokenfield('getTokens');
        var originalSource = field.data('bs.tokenfield').options.autocomplete.source; // Remove the token from the newSource
        ...
        field.data('bs.tokenfield').$input.autocomplete({
          source: newSource
        });
    ...
    setUpUserTokenFields: function setUpUserTokenFields() {
    ...
    getColumns: function getColumns(dataSourceId) {
    ...
    getUserColumns: function getUserColumns(dataSourceId) {
    ...
    updateFieldsWithColumns: function updateFieldsWithColumns(dataSourceColumns) {
      ...
      $('[name="summary_select_field"]').each(function () {
        ...
        var defaultOptions = ['<option value="none">-- Select a data field</option>', '<option disabled>------</option>', '<option value="empty">None</option>', '<option value="custom">Custom</option>', '<option disabled>------</option>'];
      ...
      $('[name="detail_select_field"]').each(function () {
        var oldValue = $(this).val();
        var defaultOptions = ['<option value="none">-- Select a data field</option>', '<option disabled>------</option>', '<option value="custom">Custom</option>', '<option disabled>------</option>'];
    ...
    updateUserFieldsWithColumns: function updateUserFieldsWithColumns(userDataSourceColumns) {
      userDataSourceColumns = userDataSourceColumns || [];
      var fOptions = [];
      var lOptions = [];
      var eOptions = [];
      var pOptions = [];
      var aOptions = []; // Get old values
      ...
      var oldAdminValue = $('.select-user-admin-holder select').val(); // Reset
      ...
      $('.select-user-admin-holder select').html(''); // Append options
      // First Name
      ...
      $('.select-user-firstname-holder select').val(oldFirstNameValue); // Last Name
      ...
      $('.select-user-lastname-holder select').val(oldLastNameValue); // Email
      ...
      $('.select-user-email-holder select').val(oldEmailValue); // Photo
      ...
      $('.select-user-photo-holder select').val(oldPhotoValue); // Admin
      ...
      $('.select-user-admin-holder select').val(oldAdminValue); // Show select fields
      ...
      if (_this.config.addPermissions === 'admins' || _this.config.editPermissions === 'admins' || _this.config.editPermissions === 'users-admins' || _this.config.detelePermissions === 'admins' || _this.config.detelePermissions === 'users-admins') {
        $('.select-user-admin-holder').removeClass('hidden');
      }

      _this.setUpUserTokenFields();
    },
    updateWithFoldersInfo: function updateWithFoldersInfo(item, from) {
      item.from = from;
      return item;
    },
    ...
    initSelect2: function initSelect2(dataSources) {
      $dataSources.select2({
        data: dataSources,
    ...
    initSecondSelect2: function initSecondSelect2(dataSources) {
    ...
    loadDataFromLayout: function loadDataFromLayout() {
      return Fliplet.DataSources.get({
        roles: 'publisher,editor',
        type: null
      }, {
    ...
    createDataSourceData: function createDataSourceData() {
    ...
    changeCreateDsButton: function changeCreateDsButton(dataSource) {
      newDataSource = dataSource;
      return _this.getColumns(dataSource.id).then(function () {
    ...
    checkSortPanelLength: function checkSortPanelLength() {
    ...
    checkFilterPanelLength: function checkFilterPanelLength() {
    ...
    addSortItem: function addSortItem(data) {
      data.columnLabel = data.column === 'none' ? '(Field)' : data.column;
      data.sortByLabel = data.sortBy === 'none' ? '(Sort)' : data.sortBy;
      data.orderByLabel = data.orderBy === 'none' ? '(Order)' : data.orderBy;
      var $newPanel = $(sortPanelTemplate(data));
      $sortAccordionContainer.append($newPanel);
    },
    addFilterItem: function addFilterItem(data) {
      data.columnLabel = data.column === 'none' ? '(Field)' : data.column;
      data.valueField = data.valueType === 'enter-value' ? 'Value' : 'Value for';
    ...
    addSummaryItem: function addSummaryItem(data) {
    ...
    addDetailItem: function addDetailItem(data) {
      data.date = moment().format('MMM Do YYYY');
      var $newPanel = $(detailsRowTemplate(data));
      $detailsRowContainer.append($newPanel);
    },
    initializeDetailViewSortable: function initializeDetailViewSortable() {
      $detailsRowContainer.sortable({
        ...
        stop: function stop(event, ui) {
          ...
          _this.config.detailViewOptions = _.concat.apply(this, _(_this.config.detailViewOptions).partition(function (option) {
    ...
    initializeSortSortable: function initializeSortSortable() {
    ...
    initializeFilterSortable: function initializeFilterSortable() {
    ...
    getSelectedRange: function getSelectedRange(editor) {
      return {
        from: editor.getCursor(true),
        to: editor.getCursor(false)
      };
    },
    autoFormatSelection: function autoFormatSelection(editor) {
      if (editor && typeof editor === 'boolean') {
        if (baseTemplateEditor) {
          ...
          baseTemplateEditor.autoFormatRange({
            ...
            line: totalLinesBaseTemplateEditor,
            ch: totalCharsBaseTemplateEditor
          }); // Remove selection
        ...
        if (loopTemplateEditor) {
          ...
          loopTemplateEditor.autoFormatRange({
            ...
            line: totalLinesLoopTemplateEditor,
            ch: totalCharsLoopTemplateEditor
          }); // Remove selection
        ...
        if (searchResultsTemplateEditor) {
          ...
          searchResultsTemplateEditor.autoFormatRange({
            ...
            line: totalLinesSearchResultsTemplateEditor,
            ch: totalCharsSearchResultsTemplateEditor
          }); // Remove selection
        ...
        if (detailTemplateEditor) {
          ...
          detailTemplateEditor.autoFormatRange({
            ...
            line: totalLinesDetailTemplateEditor,
            ch: totalCharsDetailTemplateEditor
          }); // Remove selection
        ...
        if (filterLoopTemplateEditor) {
          ...
          filterLoopTemplateEditor.autoFormatRange({
            ...
            line: totalLinesFilterLoopTemplateEditor,
            ch: totalCharsFilterLoopTemplateEditor
          }); // Remove selection
        ...
        if (otherLoopTemplateEditor) {
          ...
          otherLoopTemplateEditor.autoFormatRange({
            ...
            line: totalLinesOtherLoopTemplateEditor,
            ch: totalCharsOtherLoopTemplateEditor
          }); // Remove selection
        ...
        if (cssStyleEditor) {
          ...
          cssStyleEditor.autoFormatRange({
            ...
            line: totalLinesCssStyleEditor,
            ch: totalCharsCssStyleEditor
          }); // Remove selection
        ...
        if (javascriptEditor) {
          ...
          javascriptEditor.autoFormatRange({
            ...
            line: totalLinesJavascriptEditor,
            ch: totalCharsJavascriptEditor
          }); // Remove selection
      ...
      editor.autoFormatRange(range.from, range.to); // Remove selection
    ...
    commentSelection: function commentSelection(editor) {
      var range = _this.getSelectedRange(editor);

      editor.commentRange(true, range.from, range.to);
    },
    removeCommentSelection: function removeCommentSelection(editor) {
      var range = _this.getSelectedRange(editor);

      editor.commentRange(false, range.from, range.to);
    },
    codeMirrorConfig: function codeMirrorConfig(mode) {
      return {
        mode: {
          name: mode
        },
    ...
    getCodeEditorData: function getCodeEditorData(selectedLayout, fromReset) {
    ...
    setupCodeEditors: function setupCodeEditors(selectedLayout, fromReset) {
      ...
      return _this.getCodeEditorData(selectedLayout, fromReset).then(function () {
        var baseTemplatePromise = new Promise(function (resolve) {
          if (baseTemplateEditor) {
            ...
            baseTemplateEditor = CodeMirror.fromTextArea(baseTemplate, _this.codeMirrorConfig(baseTemplateType));
          }
        ...
        var loopTemplatePromise = new Promise(function (resolve) {
          if (loopTemplateEditor) {
            ...
            loopTemplateEditor = CodeMirror.fromTextArea(loopTemplate, _this.codeMirrorConfig(loopTemplateType));
          }
        ...
        var searchResultsTemplatePromise = new Promise(function (resolve) {
          if (searchResultsTemplateEditor) {
            ...
            searchResultsTemplateEditor = CodeMirror.fromTextArea(searchResultsTemplate, _this.codeMirrorConfig(searchResultsTemplateType));
          }
        ...
        var detailTemplatePromise = new Promise(function (resolve) {
          if (detailTemplateEditor) {
            ...
            detailTemplateEditor = CodeMirror.fromTextArea(detailTemplate, _this.codeMirrorConfig(detailTemplateType));
          }
        ...
        var filterLoopTemplatePromise = new Promise(function (resolve) {
          if (filterLoopTemplateEditor) {
            ...
            filterLoopTemplateEditor = CodeMirror.fromTextArea(filterLoopTemplate, _this.codeMirrorConfig(filterLoopTemplateType));
          }
        ...
        var otherLoopTemplatePromise = new Promise(function (resolve) {
          if (otherLoopTemplateEditor) {
            ...
            otherLoopTemplateEditor = CodeMirror.fromTextArea(otherLoopTemplate, _this.codeMirrorConfig(otherLoopTemplateType));
          }
        ...
        var cssStylePromise = new Promise(function (resolve) {
          if (cssStyleEditor) {
            ...
            cssStyleEditor = CodeMirror.fromTextArea(cssStyle, _this.codeMirrorConfig(cssStyleType));
          }
        ...
        var javascriptPromise = new Promise(function (resolve) {
          if (javascriptEditor) {
            ...
            javascriptEditor = CodeMirror.fromTextArea(javascript, _this.codeMirrorConfig(javascriptType));
          }
        ...
        return Promise.all([baseTemplatePromise, loopTemplatePromise, searchResultsTemplatePromise, detailTemplatePromise, filterLoopTemplatePromise, otherLoopTemplatePromise, cssStylePromise, javascriptPromise]).then(function () {
          _this.resizeCodeEditors();
        });
      });
    },
    resizeCodeEditors: function resizeCodeEditors() {
    ...
    resetToDefaults: function resetToDefaults(id) {
      Fliplet.Modal.confirm({
        ...
        _resetToDefaults = true; // Uncheck checkbox

        $('input#' + id).prop('checked', false).trigger('change'); // Reset settings
    ...
    saveLists: function saveLists(toReload) {
      var likesPromise;
      ...
      data.defaultData = toReload || !data.dataSourceId ? true : false; // Get sorting options
      ...
      _.forEach(_this.config.filterOptions, function (item) {
        ...
        if (item.valueType !== 'enter-value') {
          var result = _this.setSourceValue({
            key: item.fieldValue
          });
      ...
      _.forEach(_this.config['summary-fields'], function (item) {
        ...
        item.customField = $('#summary_custom_field_' + item.id).val(); // Delete unnecessary attributes before saving each item
      ...
      _.forEach(_this.config.detailViewOptions, function (item) {
        ...
        item.fieldLabelDisabled = item.fieldLabel === 'no-label'; // Delete unnecessary attributes before saving each item
        // ...including some legacy settings that are no longer supported
      ...
      data.detailViewAutoUpdate = $('input#enable-auto-update').is(':checked'); // Get search and filter
      ...
      data.searchFields = typeof $('#search-column-fields-tokenfield').val() !== 'undefined' ? $('#search-column-fields-tokenfield').val().split(',').map(function (x) {
        return x.trim();
      }) : [];
      data.sortFields = typeof $('#sort-column-fields-tokenfield').val() !== 'undefined' ? $('#sort-column-fields-tokenfield').val().split(',').map(function (x) {
        return x.trim();
      }) : [];
      data.filterFields = typeof $('#filter-column-fields-tokenfield').val() !== 'undefined' ? $('#filter-column-fields-tokenfield').val().split(',').map(function (x) {
        return x.trim();
      }) : [];
      data.filtersInOverlay = $('#enable-filter-overlay').is(':checked'); // Number of list items
      ...
      _this.config.agendaButtonsEnabled = _this.config.pollEnabled || _this.config.surveyEnabled || _this.config.questionsEnabled; // Get social feature
      ...
      data.userNameFields = typeof $('#user-name-column-fields-tokenfield').val() !== 'undefined' ? $('#user-name-column-fields-tokenfield').val().split(',').map(function (x) {
        return x.trim();
      }) : [];
      ...
      if (toReload) {
        return Promise.all([likesPromise, bookmarksPromise, commentsPromise]).then(function () {
          _this.config = data;
